...
export const name = 'qbtc';

function proposalAdapter(p: any): GovProposal {
  if (p) {
    if (p.messages && p.messages.length >= 1) p.content = p.messages[0].content || p.messages[0];
    if (p.id && !p.proposal_id) p.proposal_id = p.id;
    p.final_tally_result = {
      yes: p.final_tally_result?.yes ?? p.final_tally_result?.yes_count,
      no: p.final_tally_result?.no ?? p.final_tally_result?.no_count,
      no_with_veto: p.final_tally_result?.no_with_veto ?? p.final_tally_result?.no_with_veto_count,
      abstain: p.final_tally_result?.abstain ?? p.final_tally_result?.abstain_count,
    };
  }
  return p;
}
